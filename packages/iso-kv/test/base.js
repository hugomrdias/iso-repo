...
export const baseSchema = z.record(z.number())
/**
 * @param {import('../src/types.js').IKv<z.infer<typeof baseSchema>>} kv
...
export function baseTests(kv, suite) {
  ...
  test('should testing', async () => {
    await kv.set('name', 1)

    assert.equal(await kv.get('name'), 1)
  })
}
